...
unsigned Molecule::size() const
{
    ...
    for (unsigned r = 0; r < Molecule::NUM_UNIQUE_FRAGMENTS; r++)
    {
...
void Molecule::GetNumLinkersBricks(int& numLinkers, int& numUniqueLinkers,
                                   int& numBricks, int& numUniqueBricks) const
{
    ...
    for (unsigned r = Molecule::BRICK_INDEX_START;
         r <= Molecule::BRICK_INDEX_END; r++)
    {
        if (fragmentCounter[r] != 0)
        {
            numUniqueBricks++;
            numBricks += fragmentCounter[r];
        }
    }

    for (unsigned ell = Molecule::LINKER_INDEX_START;
         ell <= Molecule::LINKER_INDEX_END; ell++)
    {
                                  ...
                                  unsigned int numBricks, unsigned int numLinkers)
{
    ...
    Molecule::BRICK_INDEX_START = 0;
    Molecule::BRICK_INDEX_END = numBricks - 1;
    Molecule::LINKER_INDEX_START = numBricks;
    Molecule::LINKER_INDEX_END = numBricks + numLinkers - 1;
...
// std::cout << "Comparing: " << *this << " and " << that << std::endl;
    ...
    //
    for (unsigned f = 0; f < Molecule::FRAGMENT_END_INDEX; f++)
    {
                                              ...
                                              int thatAtomIndex) const
{
    ...
    // Combine all the linkers and rigids into this molecule.
    for (unsigned f = 0; f <= FRAGMENT_END_INDEX; f++)
    {
        newLocal->fragmentCounter[f] = this->fragmentCounter[f] + that.fragmentCounter[f];
    }
...
std::string Molecule::toString() const
{
    ...
    else if (IsBrick())  oss << " is a rigid.";
    ...
    else if (IsComplex())
    {
        ...
        GetNumLinkersBricks(numLinkers, numUniqueLinkers, numBricks, numUniqueBricks);

        oss << "There are ";
        oss << numBricks << " rigids, and ";
        oss << numLinkers;
